...
class ParserState:
    ...
    def get_char(self, pos: Optional[int] = None) -> Maybe[ParserValue]:
        pos = self.col_number if pos is None else pos
        if pos < len(self.val):
            return Maybe(ParserValue(self.val[pos]))
        else:
            return Maybe(None)
...
def literal(s: str, ignore_case: bool = False) -> Parser:
    ...
    def inner_str(p_val: ParserState) -> ParserTuple:
        ...
        for n, ch in enumerate(s):
            ...
            if bool(match) and icase_equals(ch, match.val):
    ...
    def inner(p_val: ParserState) -> ParserTuple:
        ...
        for parser in parsers:
            ...
            if bool(match):
                ...
                return (Maybe(get_failure(rest), True), (p_val if backtrack else rest))
...
def n_or_more(parser: Parser, n: int = 0):
    return (sequence([parser] * n) & many_of(parser)).map(lambda x: x[0] + x[1])
...
def binary_operator(func: Callable[[float, float], float], token):
    ...
    return sequence([p_number(), literal(token), p_number(), junk(space)]).map(op)
...
def kleene(parser_primary, parser_secondary=None):
...
def n_or_more(parser_primary, n=0, parser_secondary=None):
    seq = sequence([parser_primary] * n)
    k = kleene(parser_primary, parser_secondary)
...
def parse_string(pstring: str):
    ...
    seq = sequence(map(match_char, pstring))

    def inner(s):
        return map_to(joiner, seq)(s)
...
def parse_string_ignore_whitespace(pstring: str):
    ...
    seq = sequence(map(matcher, pstring))
    return seq
...
def between(sep1, interior, sep2):
    return ignore_left(sep1, ignore_right(interior, sep2))
...
def create_table():
    ...
    name = or_else(string, between(match_char("`"), string, match_char("`")))

    def phrase(s):
        return sequence(list_join(map(parse_string, s.split(" ")), space))

    def inner(s):
        create_stmt = sequence(
            [
                ignore_right(parse_string("create"), space),
                or_else(parse_string("table"), parse_string("view")),
                ignore_left(space, name),
                optional(ignore_left(space, phrase("if not exists"),)),
            ]
        )

        column = ignore_right(name, optional(match_char(",")))

        body = ignore_left(
            optional(space), between(match_char("("), kleene(column), match_char(")"))
        )
