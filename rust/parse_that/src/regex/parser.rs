...
// ── Unicode property tables (minimal) ───────────────────────────────────
...
fn unicode_category_ranges(name: &str) -> Option<Vec<CodepointRange>> {
    // For now, support the commonly referenced categories.
    // These are the same ranges that regex-syntax would produce.
    match name {
