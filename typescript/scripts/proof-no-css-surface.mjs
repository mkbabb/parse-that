...
//
// RESTORED 2026-09-23 (X.P.W6.r). The gate was retired at X.P.W5.a (4eac70c)
// when a CSS surface returned to parse-that on a `./css` subpath. The owner
// ruled on 2026-09-23 (value.js COHESION §0by), verbatim: "What you recommend.
// No custom grammar, unless it's BBNF." The CSS grammar lives in value.js,
// authored in BBNF on parse-that; parse-that is a general parsing library again.
// Legs (4) and (5) were added at restoration to cover the retired `./css` seam.
//
...
// (4) The retired `./css` seam's source is gone (src/css: the AC-1 algebra,
//     its lowerings and the ac1.wasm build).
if (existsSync(resolve(root, "src/css"))) {
    console.error("FAIL: CSS directory still present: src/css");
    failed = true;
}

// (5) The package manifest declares no CSS subpath and ships no CSS files.
const pkg = JSON.parse(readFileSync(resolve(root, "package.json"), "utf8"));
const cssExports = Object.keys(pkg.exports ?? {}).filter((k) => /css/i.test(k));
if (cssExports.length > 0) {
    console.error(`FAIL: CSS subpath still exported: ${cssExports.join(", ")}`);
    failed = true;
}
const cssFiles = (pkg.files ?? []).filter((f) => /css/i.test(f));
if (cssFiles.length > 0) {
    console.error(`FAIL: CSS paths still in package files: ${cssFiles.join(", ")}`);
    failed = true;
}
